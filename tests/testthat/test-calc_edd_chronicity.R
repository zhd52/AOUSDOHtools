test_that("calc_edd_chronicity works with valid input", {
  ...
  # Expected output: Person 1 has the maximum chronicity score (260 * 9), person 2 has a moderate score (3 * 9)
  expected_output <- data.frame(
    person_id = c(1, 2),
    edd_chronicity = c(2340, 27) # 260 * 9 = 2340, 3 * 9 = 27
  )

  # Call the function and compare with expected output
  result <- calc_edd_chronicity(survey_df)
  ...
  expect_equal(result$edd_chronicity, expected_output$edd_chronicity)
})



test_that("calc_edd_chronicity handles incomplete responses correctly", {
  ...
  # Expected output: Person 1 gets NA for incomplete responses, person 2 has the maximum score (260 * 9)
  expected_output <- data.frame(
    person_id = c(1, 2),
    edd_chronicity = c(NA_real_, 2340) # Person 1 has incomplete answers
  )
...
test_that("calc_edd_chronicity handles missing responses", {
  ...
  # Expected output: Person 1 gets NA, person 2 has the maximum score (260 * 9)
  expected_output <- data.frame(
    person_id = c(1, 2),
    edd_chronicity = c(NA_real_, 2340) # NA for person 1 with missing answers
  )
...
test_that("calc_edd_chronicity handles empty input", {
  # Empty input case
  survey_df <- data.frame(
    person_id = integer(0),
...
test_that("calc_edd_chronicity handles invalid column names", {
  ...
  expect_error(calc_edd_chronicity(bad_survey_df), "object 'question_concept_id' not found")
})
