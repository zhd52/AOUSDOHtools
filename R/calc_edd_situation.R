...
#'
#' @return A data frame with two columns: `person_id` and `edd_situation`, where `edd_situation`
  ...
  if (!is.null(survey_df)){
    df_edd_situation  <-  survey_df |>
      ...
      dplyr::mutate(value = dplyr::case_when(
        ...
        answer_concept_id %in% c(40192464, 40192453, 40192461, 40192391, 40192421) ~ 1, # A few times a year, a few times a month, ...
        TRUE ~ 999)) |>
